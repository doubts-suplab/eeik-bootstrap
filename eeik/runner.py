...
Usage:
    eeik run <generator-name> [options]           # (or: python -m eeik run <generator-name>)
...
Examples:
    ...
    eeik run repository-generator --dry-run

    # Generate repo scaffold (writes to stdout)
    eeik run repository-generator

    # Analyse project and write draft manifest
    eeik run project-analyzer --output project-manifest.yaml
...
# Prompt assembly is shared with the governed generation path via eeik.prompts — importing it here
# (rather than the reverse) keeps generation → prompts ← runner one-directional (no import cycle).
from eeik.prompts import (  # noqa: E402
    GENERATOR_REGISTRY,
    GENERATORS,
    build_prompt,
    load_manifest,
