...
halo = pytest.importorskip("halo_agent_harness", reason="halo-agent-harness not installed")
if not all(hasattr(halo, sym) for sym in ("AgentInput", "Harness", "DecisionAction")):
    pytest.skip(
        "installed 'halo_agent_harness' is not the HALO runtime (missing AgentInput/Harness API)",
...
def test_generation_is_never_auto_enforced_and_routes_to_review():
    ...
    from halo_agent_harness import AgentInput, Harness
    from halo_agent_harness.adapters.inmemory import InMemoryHumanReview, InMemoryObservability
